...
public class CommandAI extends CommandBase {
    ...
    public void handle(SlashCommandInteractionEvent e) {
        if (!Objects.equals(e.getChannelId(), Configuration.getAiChannel().get())) {
            e.reply("Prompts to the AI are only allowed in <#" + Configuration.getAiChannel().get() + ">").setEphemeral(true).queue();
            return;
        }
        ...
        var res = client.models.generateContent("gemini-2.5-flash", prompt + this.prompt, null);
        e.getHook().sendMessage(res.text()).queue();
    }
}

    ...
    private static void discord(@NotNull final LogChannels channel, @NotNull final Level level, @NotNull final String log) {
        try {
            Utils.getLogChannel(channel)
                    .sendMessage("```js\n[%s] %s\n```".formatted(level.getName().toUpperCase(),log))
                    .queue(null,
                            f -> console(SEVERE, "Failed to log message!\nChannel: %s\nMessage: %s\n Error: %s"
                                    .formatted(channel.id, log, f.getMessage())));
        } catch (Exception e) {
            console(WARNING, "Failed to log message!\nChannel: %s\nContent: %s\nException: %s"
                    .formatted(channel.id, log, e.getMessage()));
        }
    }

...
public class MemberWelcome extends ListenerAdapter {
    ...
    private final TextChannel channel = Channels.WELCOME.getChannel();
    ...
    public void onGuildMemberJoin(@NotNull final GuildMemberJoinEvent e) {
        ...
        if (Bot.database.addMember(Utils.getMember(member))) {
            channel.sendMessage(String.format(Locale.GERMAN,"**%s Welcome to the server %s!**\n-# Member: #%,d",Emoji.WAVE.formatted, member.getAsMention(),++memberCount))
                    .queue(null, f -> Logger.warn("Failed to send welcome message for user %s! %s", member.getAsMention(), f));
        } else Logger.warn("Member %s was not added to the database and a welcome message has not been sent! If they're already in the database, then you can look past this warning.", member.getAsMention());
